#!/usr/bin/env python3
"""
rule-alpha-v2.2a Walk-Forward验证
v2.2a: 增加flow_rank权重, 减少衰减因子权重
"""
...
print("="*60)
print("rule-alpha-v2.2a Walk-Forward验证")
print("="*60)
...
def score_v2_2a(day):
    ...
    s['score'] += s['lg_net_5d'].fillna(0).rank(pct=True) * 1.5
    s['score'] += (-s['ma20_bias'].fillna(0)).clip(-0.2, 0.2) * 1
    return s
...
# 3. 回测函数
# ============================================================
def run_backtest(df_all, score_fn, test_start, test_end,
...
folds = [
    (20160101, 20180101, 20180101, 20190101),
    (20170101, 20190101, 20190101, 20200101),
    (20180101, 20200101, 20200101, 20210101),
    (20190101, 20210101, 20210101, 20220101),
    (20200101, 20220101, 20220101, 20230101),
    (20210101, 20230101, 20230101, 20240101),
    (20220101, 20240101, 20240101, 20250101),
    (20230101, 20250101, 20250101, 20260616),
]
...
for config_name, score_fn in configs:
    ...
    for fold_idx, (train_start, train_end, test_start, test_end) in enumerate(folds):
        result = run_backtest(df, score_fn, test_start, test_end, dd_thresholds=DD_THRESHOLDS)
        
        if result is not None:
            fold_results.append(result)
            print(f"    Fold {fold_idx+1} ({test_start}-{test_end}): Sharpe={result['sharpe']:.2f} CAGR={result['cagr']:.1%} DD={result['max_dd']:.1%}")
        else:
            print(f"    Fold {fold_idx+1} ({test_start}-{test_end}): SKIPPED")
...
# ============================================================
output = {
    'experiment': 'rule-alpha-v2.2a-walk-forward',
...
with open('research/rule_alpha_v2_2a_walkforward.json', 'w') as f:
    json.dump(output, f, indent=2, default=str)

print(f"\n结果已保存: research/rule_alpha_v2_2a_walkforward.json")
print("="*60)
print("CEO决策: v2.2a Walk-Forward验证完成")
print("="*60)
