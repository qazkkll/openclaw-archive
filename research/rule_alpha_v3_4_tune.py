#!/usr/bin/env python3
"""v3.4: TopN sweep with 20d hold, v2.1+flow_mom scoring"""
import pandas as pd, numpy as np, json, time, warnings
warnings.filterwarnings('ignore')
import os; os.chdir(os.path.expanduser('~/.hermes/openclaw-archive'))
...
for c in ['total_net','lg_net','md_net','elg_net']: df[c] = df[c].fillna(0)
df = df[~df['sym'].str.startswith('688')].copy()
df = df[(df['close']>=3)&(df['close']<=200)&(df['volume']>0)].copy()
df = df.sort_values(['sym','date']).reset_index(drop=True)
for w in [5,10,20]: df[f'ret{w}'] = df.groupby('sym')['close'].pct_change(w)
...
def score_v21_fm(g):
    s = score_v21(g)
    s += rrank(g['flow_mom'].fillna(0).values)*1
    return s

def run_bt(score_fn, label, hold=20, sl=-1, top_n=20, warmup=180):
    sl_f=sl/100; DD_THR=[(-0.03,0.80),(-0.06,0.60),(-0.10,0.40),(-0.14,0.20),(-0.18,0.00)]
    COST=0.0015; nd=len(all_dates)
    ...
    for i in range(warmup, nd):
        ...
        for sy in list(pos.keys()):
            ...
            if len(rows)>0:
                ...
                if sl_f<0 and r<=sl_f:
                    eq+=p['shares']*cp*(1-COST/2); trades.append({'ret':r,'reason':'SL'}); del pos[sy]
                else: pv+=p['shares']*cp
            else: pv+=p['shares']*p.get('cp',p['price'])
        teq=eq+pv
        if teq>peak: peak=teq
        dd=teq/peak-1; d_eq.append(teq); d_dates.append(d)
        ...
        for sy in list(pos.keys()):
            if sy not in tgt:
                ...
                eq+=p['shares']*cp*(1-COST/2); trades.append({'ret':cp/p['price']-1,'reason':'rebal'}); del pos[sy]
        ...
        for _,row in top.iterrows():
            ...
            if cost>eq: continue
            eq-=cost; pos[sy]={'price':price,'shares':shares,'idx':i,'cp':price}
    for sy,p in list(pos.items()):
        cp=p.get('cp',p['price']); eq+=p['shares']*cp*(1-COST/2); trades.append({'ret':cp/p['price']-1,'reason':'end'})
    darr=np.array(d_eq)
    ...
    neg=dr[dr<0]; sortino=dr.mean()/neg.std()*np.sqrt(252) if len(neg)>0 and neg.std()>0 else 0
    rm=np.maximum.accumulate(darr); max_dd=(darr/rm-1).min()
    tl=pd.DataFrame(trades) if trades else pd.DataFrame()
    nt=len(tl)
    if nt>0:
        wr=(tl['ret']>0).mean(); aw=tl[tl['ret']>0]['ret'].mean() if (tl['ret']>0).any() else 0
        al=tl[tl['ret']<0]['ret'].mean() if (tl['ret']<0).any() else 0
        slr=(tl[tl['reason']=='SL'].shape[0]/nt); plr=abs(aw/al) if al!=0 else 0
    else: wr=aw=al=slr=plr=0
    return {'label':label,'cagr':round(cagr*100,1),'sharpe':round(sharpe,3),'sortino':round(sortino,3),
            'max_dd':round(max_dd*100,1),'wr':round(wr*100,1),'plr':round(plr,2),'slr':round(slr*100,0),'nt':nt}

# Sweep
results = []
for fn, label in [(score_v21,'v2.1'),(score_v21_fm,'v2.1+fm')]:
    for top_n in [15,20,25,30]:
        t1=time.time()
        r=run_bt(fn,f'{label}_T{top_n}',hold=20,sl=-1,top_n=top_n)
        if r:
            r['elapsed']=round(time.time()-t1,1); results.append(r)
            print(f"{r['label']:<25} Sharpe={r['sharpe']:.3f} CAGR={r['cagr']:.1f}% DD={r['max_dd']:.1f}% WR={r['wr']:.1f}% P/L={r['plr']:.2f} SL={r['slr']:.0f}% ({r['nt']}t)")

# Also test SL=-2% on best
best=max(results,key=lambda x:x['sharpe'])
best_fn = score_v21_fm if 'fm' in best['label'] else score_v21
best_tn = int(best['label'].split('_T')[1])
for sl in [-2,-3]:
    r=run_bt(best_fn,f"{best['label']}_SL{sl}%",hold=20,sl=sl,top_n=best_tn)
    if r:
        r['elapsed']=0; results.append(r)
        print(f"{r['label']:<25} Sharpe={r['sharpe']:.3f} CAGR={r['cagr']:.1f}% DD={r['max_dd']:.1f}% WR={r['wr']:.1f}%")

print()
rdf=pd.DataFrame(results).sort_values('sharpe',ascending=False)
print(f"{'Label':<30} {'Sharpe':>8} {'Sortino':>8} {'CAGR%':>8} {'DD%':>8}")
print('-'*65)
for _,r in rdf.iterrows():
    mark=' *' if r['sharpe']==rdf['sharpe'].max() else ''
    print(f"{r['label']:<30} {r['sharpe']:>8.3f} {r['sortino']:>8.3f} {r['cagr']:>8.1f} {r['max_dd']:>8.1f}{mark}")

with open('research/rule_alpha_v3_4_tune.json','w') as f:
