#!/usr/bin/env python3
"""
CEO Experiment: XGBoost + Rule-based Ensemble (v3 - optimized)
Compute scores once per fold, apply different ensemble weights.
"""
...
print(f"[CEO] Ensemble v3 {time.strftime('%Y-%m-%d %H:%M')}")
t0 = time.time()
...
# Rule-based scoring (vectorized)
def score_rule_vectorized(day_df):
    s = day_df.copy()
    s['rule_score'] = 0.0
...
while True:
    ...
    if test_end_dt > int_to_dt(all_dates[-1]):
        break
    train_dates = [d for d in all_dates if dt_to_int(train_start_dt) <= d <= dt_to_int(train_end_dt)]
    test_dates = [d for d in all_dates if dt_to_int(test_start_dt) <= d <= dt_to_int(test_end_dt)]
    if len(train_dates) >= 200 and len(test_dates) >= 20:
...
# Results: {strategy: {fold_idx: [port_returns]}}
all_results = {name: {'port_returns': [], 'fold_sharpes': [], 'regime_rets': {'bull': [], 'cautious': [], 'bear': []}} for name in ensemble_configs}
...
for fold_idx, fold in enumerate(folds):
    ...
    model.fit(train_data[XGB_FEATURES].fillna(0), train_data['fwd_10d'])
    
    # Test: rebalance every HOLD_DAYS
    test_dates = fold['test_dates']
    rebal_dates = test_dates[::HOLD_DAYS]
    
    # Pre-compute scores for ALL rebal dates
    rebal_scores = {}
    for rebal_date in rebal_dates:
        ...
        # XGBoost score
        day_data['xgb_score'] = model.predict(day_data[XGB_FEATURES].fillna(0))
        day_data['xgb_rank'] = day_data['xgb_score'].rank(pct=True)
        
        # Rule score
        day_data = score_rule_vectorized(day_data)
        day_data['rule_rank'] = day_data['rule_score'].rank(pct=True)
        
        # Market regime
        mb = day_data['breadth'].mean()
        mr = day_data['mkt_ret20'].mean()
        if mb > 0.5 and mr > 0: regime = 'bull'
        elif mb < 0.3 or mr < -0.05: regime = 'bear'
        else: regime = 'cautious'
        
        # Filter
        day_data = day_data[
            (day_data['close'] >= 3) & (day_data['close'] <= 200) &
            (~day_data['sym'].str.contains('ST|退市', na=False)) & (day_data['volume'] > 0)
        ]
        
        rebal_scores[rebal_date] = {'data': day_data, 'regime': regime}
    
    # Now apply different ensemble weights
    for strat_name, (rw, xw) in ensemble_configs.items():
        strat_port_returns = []
        
        for rebal_date in rebal_dates:
            if rebal_date not in rebal_scores:
                continue
            
            info = rebal_scores[rebal_date]
            day_data = info['data']
            regime = info['regime']
            
            # Ensemble score
            day_data = day_data.copy()
            day_data['ens_score'] = day_data['rule_rank'] * rw + day_data['xgb_rank'] * xw
            
            # Top N
            top = day_data.nlargest(TOP_N, 'ens_score')
            ...
            for _, row in top.iterrows():
                ...
                if len(hold) > 0:
                    min_ret = hold['close'].min() / row['close'] - 1
                    if min_ret <= SL:
                        ret = SL
            ...
            if rets:
                ...
                all_results[strat_name]['regime_rets'][regime].append(port_ret)
        
        # Fold Sharpe
        if len(strat_port_returns) > 2:
            avg = np.mean(strat_port_returns)
            std = np.std(strat_port_returns)
            fold_sharpe = (avg * 24) / (std * np.sqrt(24)) if std > 0 else 0
            all_results[strat_name]['fold_sharpes'].append(fold_sharpe)
            all_results[strat_name]['port_returns'].extend(strat_port_returns)
    
    print(f" done ({len(rebal_dates)} rebal, {time.time()-t0:.0f}s)")
...
for name, data in all_results.items():
    ...
    regime_avg = {r: np.mean(v) if v else 0 for r, v in data['regime_rets'].items()}
...
output = {
    ...
    'experiment': 'xgb_rule_ensemble_v3',
