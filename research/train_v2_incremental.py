...
"""
A股模型V2 — 增量更新方案
1. 用现有parquet数据（到2026-05-07）
2. tushare增量拉取5月7日以后的数据
3. 合并训练+信号
"""
...
df = df[df['close'] > 0]
existing_max = df['date'].max()
print(f"  现有数据: {len(df):,} 行, {df['sym'].nunique()} 股, 最新 {existing_max.date()}")

# ===== 2. tushare增量拉取 =====
print(f"\n[2] tushare增量 ({existing_max.date()} 之后)...")
inc_start = (existing_max + pd.Timedelta(days=1)).strftime('%Y%m%d')

# 获取交易日历
cal = pro.trade_cal(exchange='SSE', start_date=inc_start, end_date='20260620', is_open='1')
new_dates = sorted(cal['cal_date'].tolist())
print(f"  新增交易日: {len(new_dates)} 天")
...
if len(new_dates) > 0:
    ...
    if all_d:
        ...
        new_d['date'] = pd.to_datetime(new_d['trade_date'])
        new_d['sym'] = new_d['ts_code'].str.replace(r'\.\w+$', '', regex=True)
        new_d = new_d.rename(columns={'vol':'volume'})
        
        new_m = pd.concat(all_m, ignore_index=True)
        new_m['date'] = pd.to_datetime(new_m['trade_date'])
        new_m['sym'] = new_m['ts_code'].str.replace(r'\.\w+$', '', regex=True)
        new_m['sm_net'] = new_m['buy_sm_amount'] - new_m['sell_sm_amount']
        new_m['md_net'] = new_m['buy_md_amount'] - new_m['sell_md_amount']
        new_m['lg_net'] = new_m['buy_lg_amount'] - new_m['sell_lg_amount']
        new_m['elg_net'] = new_m['buy_elg_amount'] - new_m['sell_elg_amount']
        new_m['total_net'] = new_m['net_mf_amount']
        
        new_b = pd.concat(all_b, ignore_index=True)
        new_b['date'] = pd.to_datetime(new_b['trade_date'])
        new_b['sym'] = new_b['ts_code'].str.replace(r'\.\w+$', '', regex=True)
        
        new_df = new_d[['sym','date','open','high','low','close','volume']].merge(
            new_m[['sym','date','sm_net','md_net','lg_net','elg_net','total_net']], on=['sym','date'])
        new_df = new_df.merge(new_b[['sym','date','turnover_rate','circ_mv','total_mv']], on=['sym','date'], how='left')
        
        df = pd.concat([df, new_df], ignore_index=True)
        df = df.sort_values(['sym','date']).reset_index(drop=True)
        print(f"  增量合并: +{len(new_df):,} 行, 新总计 {len(df):,} 行")
...
# 检查是否有真实市值/换手率
has_mv = 'circ_mv' in df.columns and df['circ_mv'].notna().sum() > n * 0.5
has_tr = 'turnover_rate' in df.columns and df['turnover_rate'].notna().sum() > n * 0.5
print(f"  真实市值: {'✅' if has_mv else '❌ (用volume*close代理)'}")
print(f"  真实换手率: {'✅' if has_tr else '❌ (用volume变化率代理)'}")

turnover = df['turnover_rate'].values if has_tr else np.full(n, np.nan)
circ_mv = df['circ_mv'].values if has_mv else np.full(n, np.nan)
...
for idx in range(len(starts)):
    ...
    if has_mv:
        ...
        features['log_circ_mv'][s:e] = np.log1p(v * c)
    if has_tr:
        features['turnover_20'][s:e] = pd.Series(tr).rolling(20).mean().values
    else:
        features['turnover_20'][s:e] = pd.Series(v).pct_change().rolling(20).mean().values
...
if len(latest_df) >= 15:
    ...
    print(f"  ⚠️ 最新日只有 {len(latest_df)} 只，无法生成Top15")
