#!/usr/bin/env python3
"""
CEO: XGBoost + Rule Ensemble (v5 - O(1) price lookups)
"""
...
t0 = time.time()
print(f"[CEO] Ensemble v5 {time.strftime('%Y-%m-%d %H:%M')}")
...
# Price lookup dict for O(1) access
print("  Building price lookup...")
price_lookup = {}
for _, row in df[['sym', 'date', 'close']].iterrows():
    price_lookup[(row['sym'], row['date'])] = row['close']
print(f"  Lookup: {len(price_lookup):,} entries ({time.time()-t0:.0f}s)")
...
# Pre-compute market regime per date
print("  Market regime per date...")
...
for fold_idx, fold in enumerate(folds):
    ...
    # Pre-compute ensemble scores for all weights
    rebal_dates = fold['test_dates'][::HOLD_DAYS]
    
    # Pre-filter rebal data
    rebal_data = {}
    ...
    for strat_name, (rw, xw) in ensemble_configs.items():
        ...
        for rd in rebal_dates:
            if rd not in rebal_data:
                continue
            
            day = rebal_data[rd]
            ens = day['rule_rank'].values * rw + day['xgb_rank'].values * xw
            
            # Top N indices
            top_idx = np.argsort(ens)[-TOP_N:]
            top_syms = day.iloc[top_idx]['sym'].values
            top_prices = day.iloc[top_idx]['close'].values
            
            # Exit date
            rebal_idx = fold['test_dates'].index(rd)
            exit_idx = min(rebal_idx + HOLD_DAYS, len(fold['test_dates']) - 1)
            exit_date = fold['test_dates'][exit_idx]
            if exit_date == rd:
                continue
            
            # Portfolio return using O(1) lookup
            rets = []
            for sym, entry_price in zip(top_syms, top_prices):
                ...
                # Stop loss: check intermediate dates
                for i in range(rebal_idx + 1, exit_idx + 1):
                    intermediate_date = fold['test_dates'][i]
                    intermediate_price = price_lookup.get((sym, intermediate_date))
                    if intermediate_price is not None:
                        if intermediate_price / entry_price - 1 <= SL:
                            ret = SL
                            break
...
output = {
    ...
    'experiment': 'xgb_rule_ensemble_v5',
