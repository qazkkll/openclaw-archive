"""
LambdaMART全量训练 — CPU优化版
==============================
不采样，全量11000+只股票，float32，分块处理避免OOM
"""
...
def main():
    ...
    print(f"=== Full LambdaMART Training: {model_type} ===")
    t0 = time.time()
    ...
    # 波动率
    ret1 = g.transform(lambda x: x.pct_change(1)).astype(np.float32)
    df["vol20"] = df.groupby("sym")[ret1.name if hasattr(ret1, 'name') else "ret5"].transform(
        lambda x: x.rolling(20, min_periods=10).std()
    ).astype(np.float32) if "ret1" not in df.columns else ret1.groupby(df["sym"]).transform(
        lambda x: x.rolling(20, min_periods=10).std()
    ).astype(np.float32)

    # 用ret5的波动率代替（避免存储ret1列）
    ...
    for test_year in sorted(df["year"].unique()):
        ...
        print(f"    IC={ic:+.4f} ICIR={icir:.3f} Top5={top5:+.4f} Bot20={bot20:+.4f} "
              f"Win={win:.1%} Mono={'✅' if mono else '❌'} ({r['time_s']:.0f}s)")
    ...
    if results:
        ...
        if last_model:
            path = ROOT / f"models/us/{model_type}_lambdamart.txt"
            last_model.save_model(str(path))
            print(f"  Model: {path}")

        report = {
            "model": model_type, "timestamp": pd.Timestamp.now().isoformat(),
            "algorithm": "LambdaMART", "full_data": True,
            "results": [{k: (int(v) if isinstance(v, (np.integer,)) else
                           float(v) if isinstance(v, (np.floating,)) else v)
                         for k, v in r.items()} for r in results],
            "summary": {k: round(float(avg(k)), 4) for k in ["ic", "icir", "top5", "bot20", "win"]},
            "monotonic": bool(mono_all),
        }
        rpath = ROOT / f"data/lambdamart_{model_type}_full.json"
        with open(rpath, "w") as f:
            json.dump(report, f, indent=2)
        print(f"  Report: {rpath}")

        # 9. 自动生成Manifest（代码级契约）
        try:
            sys.path.insert(0, str(ROOT / "scripts" / "us"))
            from contract.manifest import create_manifest
            create_manifest(
                model_name=model_type,
                icir=round(float(avg("icir")), 4),
                ic=round(float(avg("ic")), 6),
                train_rows=len(df),
                train_stocks=df["sym"].nunique(),
                notes=f"Auto-generated by retrain_full.py, {'PASSED' if mono_all and avg('icir') > 0.3 else 'PARTIAL'}"
            )
        except Exception as e:
            print(f"  ⚠️ Manifest generation failed: {e}")

        if mono_all and avg("icir") > 0.3:
            print(f"\n✅ PASSED")
        elif mono_all:
            print(f"\n⚠️ PARTIAL (monotonic, ICIR<0.3)")
        else:
