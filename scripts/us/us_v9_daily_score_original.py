...
def main():
    ...
    for code, sd in data.items():
        n = len(sd['c'])
        pos = n - 1  # Second-to-last = latest complete day
        
        if pos < 100: continue
        
        ct = float(sd['c'][pos-1])
        if ct < 0.5 or ct > 30: continue
    ...
    print(f'候选池: {len(scored)}只 ($0.5-30低价股)')
    ...
    for i, s in enumerate(scored[:10]):
        print(f'  {i+1}. {s["sym"]:6s} prob={s["prob"]:.4f}  ${s["price"]:.2f}  {s["sector"]}')
