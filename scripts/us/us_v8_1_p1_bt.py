...
BASE='/home/hermes/.hermes/openclaw-archive'; ML=f'{BASE}/ml'; MD=f'{BASE}/data/models'; VER='us_v8_1'
...
for i in range(0,n_total,n_batch):
    ...
    calib=cal.predict(raw.reshape(-1,1)).ravel()
...
results=[]
if os.path.exists('/home/hermes/.hermes/openclaw-project/data/models/us_v8_1_p1_bt.json'):
    print('发现已有结果，加载继续...')
    existing=json.load(open('/home/hermes/.hermes/openclaw-project/data/models/us_v8_1_p1_bt.json'))
            ...
            for R in RV:
                ...
                if len(results)%10==0:
                    json.dump({'results':results,'time':time.strftime('%Y-%m-%d %H:%M')},
                              open('/home/hermes/.hermes/openclaw-project/data/models/us_v8_1_p1_bt.json','w'))
