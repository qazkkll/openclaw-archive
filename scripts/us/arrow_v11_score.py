...
"""
绿箭V12 每日评分脚本（全市场版）
扫描全市场$1-$10股票 → 42维特征 → XGBoost排名 → Top-5+信号分级
...
def score_all():
    ...
    print("🎯 绿箭V12 全市场评分 ($1-$10)", flush=True)
    print("="*50, flush=True)
    
    # 加载数据（只取最近150天，特征最长窗口126天+buffer）
...
def main():
    ...
    if args.json:
        ...
        output = {
            'timestamp': now, 'model': 'arrow_v12',
            'total_scanned': len(latest), 'top_n': args.top, 'picks': picks
        }
        print(json.dumps(output, indent=2))
    else:
        print(f"🎯 绿箭V12 选股报告 ({now})", flush=True)
        print(f"{'='*60}", flush=True)
        print(f"模型: V12排名 (42维特征, 5天Top-5, $1-$10, 全市场)", flush=True)
    ...
    with open(compat_path, 'w') as f:
        json.dump({
            'timestamp': now, 'model': 'arrow_v12', 'total': len(latest),
