...
def load_universe():
    """从 central_config 或 latest scored 文件加载交易宇宙。"""
    # 尝试读最新的 scored 文件获取 ticker 列表
    patterns = [
        str(PROJECT_ROOT / "data" / "us" / "*scored*.json"),
        str(PROJECT_ROOT / "data" / "us" / "universe*.json"),
    ]
    tickers = set()
    for pattern in patterns:
        for f in sorted(glob.glob(pattern)):
            ...
            if isinstance(d, dict) and "picks" in d:
                ...
                tickers.update(d if isinstance(d[0], str) else [x.get("symbol", x.get("sym", "")) for x in d])
