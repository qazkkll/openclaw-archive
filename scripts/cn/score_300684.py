...
target = td[td['sym']=='300684']
if len(target) == 0:
    print("300684 not in universe")
else:
    r = target.iloc[0]
    score = r['score']
    if score >= 0.08: sig = 'GG'
    elif score >= 0.06: sig = 'G'
    elif score >= 0.04: sig = 'Y'
    else: sig = 'below'

    rank = (td['score'] >= score).sum()
    total = len(td)

    print("300684 Score: {:.4f}  Signal: {}".format(score, sig))
    print("Rank: {}/{} (top {:.1f}%)".format(rank, total, rank/total*100))
    print()
    print("Key features vs market:")
    for f in ['ret5','ret10','ret20','ma20_bias','ma60_bias','rsi_14','macd_hist','vol_ratio','lg_net_5d_rk']:
        val = r[f]
        avg = td[f].mean()
        pctl = (td[f] < val).mean() * 100
        print("  {:>18}: {:>8.4f}  (avg={:.4f}, pctl={:.0f}%)".format(f, val, avg, pctl))
