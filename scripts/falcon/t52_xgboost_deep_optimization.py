"""
T5.2: XGBoost Deep Optimization — Ultra-fast (5 configs)
"""
...
t_total = time.time()
print("=" * 70)
print("  T5.2 XGBoost Deep Optimization (Ultra-Fast)")
print("=" * 70)
...
#  5 FOCUSED CONFIGS
# ═══════════════════════════════════════════════════════════════════
XGB_P = dict(n_estimators=100, max_depth=3, learning_rate=0.05,
...
LGBM_P = dict(objective='regression', metric='mse', boosting_type='gbdt',
              num_leaves=15, learning_rate=0.05, feature_fraction=0.7,
              bagging_fraction=0.7, bagging_freq=5, min_child_samples=50,
              reg_alpha=1.0, reg_lambda=10.0, n_estimators=100,
              random_state=42, n_jobs=-1, verbose=-1)

configs = [
    ('A_xgb_strong', strong_features, 'fwd_ret_10d', 8, 'xgb', XGB_P, 'XGB strong11 (baseline ref)'),
    ('B_xgb_eng',    feat_eng,        'fwd_ret_10d', 8, 'xgb', XGB_P, 'XGB + engineered features'),
    ('C_lgbm_strong', strong_features, 'fwd_ret_10d', 8, 'lgbm', LGBM_P, 'LightGBM strong11'),
    ('D_ridge',       strong_features, 'fwd_ret_10d', 8, 'ridge', {'alpha': 1.0}, 'Ridge strong11'),
    ('E_elasticnet',  strong_features, 'fwd_ret_10d', 8, 'elasticnet', {'alpha': 0.1, 'l1_ratio': 0.5, 'max_iter': 1000}, 'ElasticNet strong11'),
]

all_results = []

for name, fc, tgt, hd, mt, mp, desc in configs:
    if mt == 'lgbm' and not HAS_LGBM:
        print(f"\n  Skipping {name}: LightGBM not installed")
        continue
    ...
    try:
        ...
        all_results.append({'config_name': name, 'error': str(e)})

# ═══════════════════════════════════════════════════════════════════
#  ENSEMBLE: XGB + Ridge (combine existing predictions)
# ═══════════════════════════════════════════════════════════════════
print(f"\n{'='*60}")
print("  Ensemble: XGB + Ridge (re-run predictions)")
print(f"{'='*60}")

try:
    xgb_r = run_wf(feat_eng, 'fwd_ret_10d', 8, 'xgb', XGB_P, 'ens_xgb')
    ridge_r = run_wf(strong_features, 'fwd_ret_10d', 8, 'ridge', {'alpha': 1.0}, 'ens_ridge')
    
    all_d = sorted(set(xgb_r.keys()) & set(ridge_r.keys()))
    ens = {}
    for d in all_d:
        ens[d] = {}
        ts = set(xgb_r.get(d, {}).keys()) | set(ridge_r.get(d, {}).keys())
        for t in ts:
            ens[d][t] = 0.5 * xgb_r.get(d, {}).get(t, 0) + 0.5 * ridge_r.get(d, {}).get(t, 0)
    
    re, ri = evaluate(ens, 'fwd_ret_10d', 8, 'F_ensemble')
    all_results.append({
        'config_name': 'F_ensemble', 'description': 'XGB+Ridge 50/50 avg, ret10d hold8',
        'model_type': 'ensemble', 'n_features': len(feat_eng),
