#!/usr/bin/env python3
"""
🦅 Falcon V0.4.1 Fixed Validation: Addressing 3 BLOCKER Issues
================================================================
修复V0.4.1审计发现的三个BLOCKER问题:

1. 实现真正的rank_inversion测试:
   - 在Walk-Forward的每个窗口中
   - 计算Top5%股票的平均收益
   - 计算Bottom20%股票的平均收益
   - 检查Top5%收益 > Bottom20%收益
   - 记录每个窗口的rank_inversion结果
...
3. 记录Sharpe退化:
   - 计算早期窗口(2017-2020)的平均Sharpe
   - 计算近期窗口(2021-2026)的平均Sharpe
   - 记录退化比例
   - 判断是否为阻断项（退化>50%为WARN，>75%为FAIL）

Walk-Forward参数:
  train_years=0.5 (6个月), test_months=6, hold_days=30
  top_n=10, cost=0.001, stop_loss=-0.15

输出: data/falcon/v041_fixed_validation_results.json
"""
...
OUTPUT_PATH = DATA_DIR / "v041_fixed_validation_results.json"

# ═══════════════════════════════════════════════════
#  因子组定义 (从v041_optimization.py复制)
# ═══════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════
#  Walk-Forward 回测 (包含所有窗口)
# ═══════════════════════════════════════════════════

def run_walk_forward_with_all_windows(ranks, prices, weights, 
                                       train_years=0.5, test_months=6,
                                       hold_days=30, top_n=10, 
                                       cost=0.001, stop_loss=-0.15):
    """
    运行Walk-Forward，确保包含所有有效窗口（不跳过2018-2020）
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("🦅 Falcon V0.4.1 Fixed Validation: Addressing 3 BLOCKER Issues")
    print("=" * 80)
    ...
    # ═══════════════════════════════════════════════
    #  问题1: 真正的Rank Inversion测试
    # ═══════════════════════════════════════════════
    print("\n" + "=" * 80)
    print("🔍 问题1: 真正的Rank Inversion测试 (Top5% vs Bottom20%)")
    print("=" * 80)
    ...
    # ═══════════════════════════════════════════════
    #  问题2: Walk-Forward (包含所有窗口)
    # ═══════════════════════════════════════════════
    print("\n" + "=" * 80)
    print("📊 问题2: Walk-Forward (包含所有有效窗口)")
    print("=" * 80)
    
    wf_result, wf_windows = run_walk_forward_with_all_windows(
        ranks, price_pivot, weights,
        train_years=0.5, test_months=6,
        hold_days=30, top_n=10, cost=0.001, stop_loss=-0.15
    )
    ...
    output = {
        "metadata": {
            ...
            "task": "V0.4.1 Fixed Validation: Addressing 3 BLOCKER Issues",
            "config": {
                "train_years": 0.5,
                "test_months": 6,
                "hold_days": 30,
    ...
    print("📋 最终摘要")
    print("=" * 80)
    print(f"  问题1 (Rank Inversion): {'✅ PASS' if ri_result and ri_result['overall_passed'] else '❌ FAIL'}")
    print(f"  问题2 (窗口排除): {'✅ PASS' if wf_result and wf_result['failed_windows'] == 0 else '⚠️ WARN'}")
    print(f"  问题3 (Sharpe退化): {degradation['status']}")
    print(f"  WF Sharpe: {wf_result['sharpe']:.3f}" if wf_result else "  WF Sharpe: N/A")
