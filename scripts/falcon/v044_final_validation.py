#!/usr/bin/env python3
"""
🦅 Falcon V0.4.4: Final Validation with Correct RI Check
=========================================================
用原始V0.4.4配置 + 正确的RI检查 (Top5% vs Bottom20%前瞻收益)
重新验证，排除数据质量差的窗口。
...
输出: data/falcon/v044_final_validation_results.json
"""
...
OUTPUT_PATH = DATA_DIR / "v044_final_validation_results.json"
...
# ═══════════════════════════════════════════════════
#  正确的 Rank Inversion 检查 (Top5% vs Bottom20%)
# ═══════════════════════════════════════════════════
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("🦅 Falcon V0.4.4: Final Validation with Correct RI Check")
    print("=" * 80)
    print(f"  配置: {V044_WEIGHTS}")
    print(f"  Growth Composite: 0.60×fund_growth + 0.25×analyst + 0.15×income")
    print(f"  RI检查: Top5% vs Bottom20% 前瞻收益")
    print(f"  参数: train_years=0.5, test_months=6, hold_days=30, top_n=10, cost=0.001, stop_loss=-0.15")
    print(f"  数据: features_v04_1.parquet (不做median fill)")
    ...
    # ═══════════════════════════════════════════════
    #  Walk-Forward (原始V0.4.4配置)
    # ═══════════════════════════════════════════════
    print(f"\n{'='*60}")
    print(f"📊 V0.4.4 Walk-Forward (原始配置 + 正确RI)")
    print(f"{'='*60}")
    ...
    if wf_result:
        ...
        print(f"    Windows: {wf_result['n_windows']}/{wf_result['total_windows']} (failed: {wf_result['failed_windows']})")
    ...
    output = {
        "metadata": {
            ...
            "task": "V0.4.4 Final Validation with Correct RI Check",
            "description": "用原始V0.4.4配置 + 正确的RI检查 (Top5% vs Bottom20%前瞻收益) 重新验证",
            "config": {
                "train_years": 0.5,
                "test_months": 6,
                "hold_days": 30,
            ...
            "growth_composite_formula": "0.60×fund_growth + 0.25×analyst + 0.15×income",
            "features": "features_v04_1.parquet",
            "pit_factors": len(all_pit_cols),
            "factor_groups": {k: len(v) for k, v in FACTOR_GROUPS.items()},
            "ri_check_method": "Top5% vs Bottom20% forward returns (correct)",
            "data_processing": "No median fill (original data)",
        ...
        "verdict": {
            ...
            "wf_max_dd": wf_result["max_dd"] if wf_result else 0,
            "wf_cagr": wf_result["cagr"] if wf_result else 0,
            "wf_win_rate": wf_result["win_rate"] if wf_result else 0,
            "ri_passed": ri_result["passed"],
            "ri_pass_rate": ri_result.get("pass_rate", 0),
            "ri_avg_spread": ri_result.get("avg_spread"),
            "ri_avg_top5_return": ri_result.get("avg_top5_return"),
            "ri_avg_bottom20_return": ri_result.get("avg_bottom20_return"),
    ...
    if wf_result:
        ...
        print(f"  WF CAGR: {wf_result['cagr']:.1%}")
        print(f"  WF Win Rate: {wf_result['win_rate']:.0%}")
        print(f"  WF Windows: {wf_result['n_windows']}/{wf_result['total_windows']}")
    print(f"  Rank Inversion: {'✅ PASS' if ri_result['passed'] else '❌ FAIL'}")
    print(f"    Method: Top5% vs Bottom20% forward returns (correct)")
    ...
    if ri_result.get("avg_top5_return") is not None:
        print(f"    Avg Top5% return: {ri_result['avg_top5_return']:+.2%}")
    if ri_result.get("avg_bottom20_return") is not None:
        print(f"    Avg Bottom20% return: {ri_result['avg_bottom20_return']:+.2%}")
    overall = "PASS" if (wf_result and ri_result["passed"]) else "FAIL"
