#!/usr/bin/env python3
"""
R2K 扩展回测 V2: 只用数据完整的R2K股票
==========================================
V1问题：用了全部691只R2K，43%无分析师数据 → 信号噪音大
V2修复：只用4/4覆盖(393只) + SPX(476只) = 869只universe
...
def load_data():
    ...
    r2k_good = {t for t, s in r2k_quality.items() if s >= 3}

    # R2K price pivot (filtered)
    all_prices = []
    ...
    print(f"  R2K 3/4覆盖: {len(r2k_good)} tickers")
...
def merge_and_run(spx_ranks, r2k_ranks, spx_prices, r2k_prices, mode="merged"):
    ...
    for wi, (train_dates, test_dates) in enumerate(windows):
        ...
        for test_date in test_dates:
            ...
            # In merged mode: SPX has cashflow, R2K doesn't → score each stock
            # using only the factors it has, with proportional weight normalization
            score_cols = []
...
def main():
    print("=" * 60)
    print("🧪 R2K Extension Backtest V2 (Quality-Filtered)")
    print("=" * 60)
    t0 = time.time()
    ...
    # Run 3 scenarios
    results = {}
    results["spx_only"] = merge_and_run(spx_ranks, r2k_ranks, spx_prices, r2k_prices, "spx_only")
    results["r2k_only"] = merge_and_run(spx_ranks, r2k_ranks, spx_prices, r2k_prices, "r2k_only")
    results["merged"] = merge_and_run(spx_ranks, r2k_ranks, spx_prices, r2k_prices, "merged")

    # Comparison table
    print("\n" + "=" * 60)
    print("📊 FINAL COMPARISON")
    print("=" * 60)
    print(f"{'Metric':<25} {'SPX Only':>12} {'R2K Only':>12} {'Merged':>12}")
    print("-" * 61)

    for key in ["trades", "win_rate", "avg_top_return", "avg_ls_spread", "sharpe_annual", "max_dd"]:
        label = {
            "trades": "Trades",
            "win_rate": "Win Rate",
            "avg_top_return": "Avg Top10 Return (%)",
            "avg_ls_spread": "L/S Spread (%)",
            "sharpe_annual": "Annual Sharpe",
            "max_dd": "Max Drawdown (%)",
        }[key]
        ...
        for mode in ["spx_only", "r2k_only", "merged"]:
            ...
            if r:
                ...
                if key == "win_rate":
                    vals.append(f"{v:.1%}")
                elif key in ("avg_top_return", "avg_ls_spread", "max_dd"):
                    vals.append(f"{v:+.2f}%")
                elif key == "sharpe_annual":
                    vals.append(f"{v:.3f}")
                else:
                    vals.append(str(v))
            else:
    ...
    output = {
        ...
        "config": {"hold_days": HOLD_DAYS, "top_n": TOP_N, "train_window": 504, "test_window": 126},
        "r2k_quality": {"total": 691, "full_coverage_4_4": len(r2k_full), "universe_size": len(r2k_full) + 476},
        "results": {k: v for k, v in results.items() if v},
    }
    out_path = PROJECT_ROOT / "data" / "falcon" / "r2k_backtest_v2_results.json"
