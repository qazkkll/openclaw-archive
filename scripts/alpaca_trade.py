...
用法:
    python3 scripts/alpaca_trade.py status      # 查看账户+持仓
    python3 scripts/alpaca_trade.py signals     # 查看今日信号
    python3 scripts/alpaca_trade.py execute     # 执行信号（买🟢🟢）
    python3 scripts/alpaca_trade.py rebalance   # 轮换持仓（卖出到期/不在信号的）
    python3 scripts/alpaca_trade.py full        # 执行+轮换一步到位
    python3 scripts/alpaca_trade.py history     # 交易历史
...
def load_latest_signals(model_name="arrow_v12"):
    """加载最新评分结果。"""
    pattern = str(DATA_DIR / f"{model_name}_scored_*.json")
    files = sorted(glob.glob(pattern))
    if not files:
        # 也尝试 blueshield
        pattern = str(DATA_DIR / f"blueshield_v10_scored_*.json")
        files = sorted(glob.glob(pattern)
)
    if not files:
        return None, []
...
def cmd_signals():
    ...
    for model_name in ["arrow_v12", "blueshield_v10"]:
        filename, picks = load_latest_signals(model_name)
        if not picks:
            print(f"⚠️ {model_name}: 无信号文件")
            continue

        print(f"\n{'='*60}")
        print(f"📡 {model_name} 信号 (来源: {filename})")
        print(f"{'='*60}")
...
# ── 命令: execute ──
def cmd_execute(client, model_name="arrow_v12", dry_run=False):
    """执行信号：买入🟢🟢信号的股票。"""
...
# ── 命令: full ──
def cmd_full(client, model_name="arrow_v12", hold_days=10, stop_loss_pct=-15.0, dry_run=False):
    """完整流程：先轮换卖出，再买入新信号。"""
    print("🔄 完整交易流程")
...
def main():
    ...
    parser.add_argument("command", choices=["status", "signals", "execute", "rebalance", "full", "history"],
                        help="执行命令")
    parser.add_argument("--model", default="arrow_v12", help="模型名 (arrow_v12 / blueshield_v10)")
